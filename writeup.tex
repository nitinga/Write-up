...
In the Tables~\ref{MAE},~\ref{Time} the mean absolute error and the run time averaged over the categories of size, rank and sparsity can be noted. Mean absolute error is the absolute value of the average error on the unknown entries. Run time is measured in seconds and all simulations were run at University of Michigan's CAEN lab computers (8GB memory with one core i7 processors). The last columns in the Tables~\ref{MAE},~\ref{Time} contains the averaged mean absolute error and the run time for the different ranks and sparsity of the synthetically generated matrices of size 1000 and 5000.
